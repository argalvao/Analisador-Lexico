...
public class Main {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		try {
			File directory = new File("input"); // Identifica o diret√≥rio de entrada de arquivos
			if (directory.isDirectory()) {
				File outputDirectory = new File("output");
				// noinspection ResultOfMethodCallIgnored
				outputDirectory.mkdir();
				File fileList[] = directory.listFiles(); // Lista todos os arquivos do diret√≥rio e armazena em um array
				for (int counter = 0; counter < fileList.length; counter++) {
					if (fileList[counter].getName().contains("entrada")) {
						...
						if(SynthaticAnalyzer.getInstance().getList().isEmpty()) {
							System.out.println("CÛdigo n„o apresenta erros, parabÈns!");
						}
						printWriter.close();
					}
				}
